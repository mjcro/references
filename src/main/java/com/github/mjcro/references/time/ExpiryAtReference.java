...
public interface ExpiryAtReference {
    static <T extends ExpiryAtReference> Collection<T> filterNonExpired(Collection<T> source) {
        return filterNonExpired(source, Instant::now);
    }

    static <T extends ExpiryAtReference> Collection<T> filterNonExpired(Collection<T> source, Supplier<Instant> supplier) {
